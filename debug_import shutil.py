...
async def export_report(filename: str, format: str = "md"):
    ...
    if format == "md":
        ...
        if not DOCX_AVAILABLE:
            raise HTTPException(501, "DOCX экспорт недоступен: установите python-docx")
        return await export_to_docx(content, filename)
    elif format == "pdf":
        if not WEASYPRINT_AVAILABLE:
            raise HTTPException(501, "PDF экспорт недоступен: установите weasyprint")
        return await export_to_pdf(content, filename)
    else:
        raise HTTPException(400, f"Формат {format} не поддерживается. Доступные: md, txt, html, docx, pdf")


async def export_to_docx(markdown_content: str, original_filename: str) -> StreamingResponse:
    """Конвертирует Markdown в DOCX с сохранением форматирования таблиц и стилей."""
    ...
    while i < len(lines):
        ...
        # Таблицы
            ...
            if i < len(lines):
                ...
                for cell in sep_line.split('|')[1:-1]:
                    ...
                    if cell.startswith(':') and cell.endswith(':'):
                        current_table_align.append('center')
                    elif cell.endswith(':'):
                        current_table_align.append('right')
                    else:
                        current_table_align.append('left')
            ...
            if table_rows:
                create_docx_table(doc, table_rows, current_table_align)
            in_table = False
            table_rows = []
            current_table_align = []
            continue  # не увеличиваем i, так как уже на следующей строке
        
        # Обычный текст
        elif stripped and not stripped.startswith('---'):
            # Обработка inline форматирования (bold, italic)
            p = doc.add_paragraph()
    ...
    docx_filename = original_filename.replace('.md', '.docx')
...
def create_docx_table(doc: Document, rows: list, align: list):
    """Создаёт таблицу в DOCX из распарсенных строк."""
    if not rows:
        return
    
    num_cols = len(rows[0][1]) if rows[0][1] else 0
    if num_cols == 0:
        return
    ...
    # Настройка ширины колонок (авто)
    for row_idx, (row_type, cells) in enumerate(rows):
        for col_idx, cell_text in enumerate(cells):
            if col_idx < num_cols:
                ...
                # Выравнивание
                if col_idx < len(align):
                    if align[col_idx] == 'center':
                        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
                    elif align[col_idx] == 'right':
...
def add_formatted_text(paragraph, text: str):
    """Добавляет текст с поддержкой **bold**, *italic*, `code`."""
    import re
...
async def export_to_pdf(markdown_content: str, original_filename: str) -> StreamingResponse:
...
{html_content}
    ...
    pdf_filename = original_filename.replace('.md', '.pdf')
    return StreamingResponse(
        buffer,
